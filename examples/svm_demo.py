...
def plot_classifier_results(history: dict, y_true: np.ndarray, y_pred: np.ndarray,
    ...
    axes[0].text(0.1, 0.5, summary_text, transform=axes[0].transAxes, fontsize=10*text_size_scale,
                 verticalalignment='center', bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.5))
    axes[0].set_xlim(0, 1)
    axes[0].set_ylim(0, 1)
    axes[0].set_title(f"{mouse_id}-{session_id} {model_name}", fontsize=14*text_size_scale)
    axes[0].axis('off')
    ...
    axes[1].set_title('Confusion Matrix', fontsize=14*text_size_scale)
...
def plot_temporal_predictions(session: SessionData, event_column: str, time_bins: np.ndarray,
                             ...
                             show_trial_boundaries: bool = False, text_size_scale: float = 1.0,
    ...
    ax1.set_title(f"{mouse_id}-{session_id} {model_name} Temporal Predictions", fontsize=14*text_size_scale)
...
def run_svm_demo(mouse_id: str, session_id: str, experiment: str,
    ...
    if use_trial_split:
        ...
        print(f"\n{step_num+1}. Balanced temporal stratification (train: {train_size:.1f}, val: {val_size:.1f}, test: {1-train_size-val_size:.1f})...")
    ...
    plot_classifier_results(history, y_true, y_pred, y_prob, "SVM Classifier", mouse_id, session_id, event_column, text_size_scale, show_legends)
    
    # Generate temporal prediction visualization
    print(f"\n{step_num+6}. Generating temporal prediction visualization...")
    
    # Create bin-wise predictions by mapping sequence predictions back to time bins
    # We'll use all sequences (train + val + test) to get full timeline coverage
    all_seq_indices = np.concatenate([train_indices, val_indices, test_indices])
    all_sequences = sequences[all_seq_indices]
    all_seq_labels = sequence_labels[all_seq_indices]
    
    # Get predictions for all sequences using SVM
    X_all_flat = classifier._flatten_sequences(all_sequences)
    X_all_scaled = classifier.scaler.transform(X_all_flat)
    
    all_preds = classifier.model.predict(X_all_scaled)
    all_probs = classifier.model.predict_proba(X_all_scaled)[:, 1]  # Probability of positive class
    
    # Map sequence predictions back to time bins
    # Initialize with NaN (for bins not covered by any sequence)
    ...
    # For each sequence, assign its prediction to the corresponding time bins
    for i, seq_idx in enumerate(all_seq_indices):
        ...
        for bin_idx in range(start_bin, min(end_bin, len(time_bins))):
            y_pred_bins[bin_idx] = all_preds[i]
            y_prob_bins[bin_idx] = all_probs[i]
    
    # Remove NaN values (bins not covered by any sequence)
    valid_mask = ~np.isnan(y_pred_bins)
    y_true_bins_clean = y_true_bins[valid_mask]
    y_pred_bins_clean = y_pred_bins[valid_mask].astype(int)
    y_prob_bins_clean = y_prob_bins[valid_mask]
    time_bins_clean = time_bins[valid_mask]
    
    print(f"Temporal predictions: {len(time_bins_clean)}/{len(time_bins)} bins covered")
    
    # Plot temporal predictions
    plot_temporal_predictions(
        session, event_column, time_bins_clean,
        y_true_bins_clean, y_pred_bins_clean, y_prob_bins_clean,
        mouse_id, session_id, "SVM Classifier",
        show_trial_boundaries=use_trial_split, text_size_scale=text_size_scale,
        show_legends=show_legends
    )
...
if __name__ == "__main__":
    ...
    run_svm_demo(
        ...
        text_size_scale=2.0,  # Make text twice as big for presentations
        show_legends=False  # Hide legends for cleaner presentation slides
    )